...
public class OpcionServicio {
    ...
    public List<RespuestaOpcion> obtenerArbol() {
        return opcionRepositorio.buscarArbol().stream()
                .map(o -> aRespuesta(o, true)).toList();
    }

    @Transactional(readOnly = true)
    public List<RespuestaOpcion> listar() {
        return opcionRepositorio.findAll().stream()
                .map(o -> aRespuesta(o, false)).toList();
    }
    ...
    public RespuestaOpcion crear(SolicitudOpcion solicitud) {
        if (opcionRepositorio.existsByCodigo(solicitud.codigo())) {
            throw new PeticionInvalidaExcepcion("Ya existe una opción con código: " + solicitud.codigo());
        }
        Opcion padre = resolverPadre(solicitud.idPadre());
        Opcion opcion = Opcion.builder()
                ...
                .build();
        return aRespuesta(opcionRepositorio.save(opcion), false);
    }
    ...
    public RespuestaOpcion actualizar(Long id, SolicitudOpcion solicitud) {
        ...
        if (!opcion.getCodigo().equals(solicitud.codigo())
                && opcionRepositorio.existsByCodigo(solicitud.codigo())) {
            throw new PeticionInvalidaExcepcion("Ya existe una opción con código: " + solicitud.codigo());
        }
        Opcion padre = resolverPadre(solicitud.idPadre());
        opcion.setCodigo(solicitud.codigo());
        opcion.setNombre(solicitud.nombre());
        opcion.setIcono(solicitud.icono());
        opcion.setRuta(solicitud.ruta());
        opcion.setPadre(padre);
        opcion.setOrdenVisualizacion(solicitud.ordenVisualizacion());
        return aRespuesta(opcionRepositorio.save(opcion), true);
    }
